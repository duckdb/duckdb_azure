...
ifneq ("${VCPKG_TOOLCHAIN_PATH}", "")
	TOOLCHAIN_FLAGS:=${TOOLCHAIN_FLAGS} -DVCPKG_MANIFEST_DIR='${PROJ_DIR}' -DVCPKG_BUILD=1 -DCMAKE_TOOLCHAIN_FILE='${VCPKG_TOOLCHAIN_PATH}' -DDUCKDB_EXPLICIT_PLATFORM='${DUCKDB_PLATFORM}'
endif
...
#### Main build
# For regular CLI build, we link the azure extension directly into the DuckDB executable
...
#### Client tests
DEBUG_EXT_PATH='$(PROJ_DIR)build/debug/extension/azure/azure.duckdb_extension'
RELEASE_EXT_PATH='$(PROJ_DIR)build/release/extension/azure/azure.duckdb_extension'
